...
struct sCell {
  ...
  glm::vec3 getVelocityAt(const glm::vec3 &p) {
    ...
    if (dx < 0.f or dx > 1.f or dy < 0.f or dy > 1.f or dz < 0.f or dz > 1.f)
      printf("ERROR:: in sCell::getVelocityAt() with dx = %.3f dy = %.3f dz = %.3f\n", dx, dy, dz);
...
class sGrid {
public:
	sGrid(glm::vec3 corner = glm::vec3(0, 0, 0), float scale = 0.1);
	~sGrid();
  ...
  const sCell& at(const glm::vec3 &idx) {
...
protected:
	...
	std::vector<std::vector<std::vector<sCell> > > m_cells;
