...
sGrid::sGrid(glm::vec3 corner, float scale) : m_corner(corner),
m_scale(scale),
...
m_sizeZ(1)
{
	m_cells = std::vector<std::vector<std::vector<sCell> > >(1, std::vector<std::vector<sCell> >(1, std::vector<sCell>(1)));
  m_searchRadius = 1;
...
bool sGrid::insertParticles(const std::vector<Particle*>::iterator & begin, const std::vector<Particle*>::iterator & end)
{
	for (auto it = begin; it != end; it++) {
		...
		while (x >= m_corner.x + m_sizeX * m_scale - 0.1f) {
			m_cells.push_back(std::vector<std::vector<sCell> >(m_sizeY, std::vector<sCell>(m_sizeZ)));
			m_sizeX++;
		}
		while (y >= m_corner.y + m_sizeY * m_scale - 0.1f) {
			for (auto& vecX : m_cells) {
				vecX.push_back(std::vector<sCell>(m_sizeZ));
			}
			m_sizeY++;
		}
...
std::vector<glm::vec3> sGrid::getFluidSnowBoundary(int density) {
...
void sGrid::sphGridInteraction(const float timeStep) {
...
void sGrid::addSmokeForce(float timeStep) {
...
void sGrid::solvePressure(float timeStep) {
...
void sGrid::correctVelocity(float timeStep) {
...
void sGrid::updateDensityAndSpeed(float timeStep) {
...
std::vector<glm::vec4> sGrid::adhereSnowSmoke(float timeStep) {
...
void sGrid::computeFluidSmokeBoundary() {
  for (int x = 0; x < m_sizeX; x++)
    for (int y = 0; y < m_sizeY; y++)
      for (int z = 0; z < m_sizeZ; z++) {
